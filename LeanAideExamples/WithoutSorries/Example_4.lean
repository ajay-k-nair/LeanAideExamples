...
-- ## LeanCode generated by LeanAide
  theorem seq_eq_const_of_zero_eq_and_succ_eq_self :
      ∀ {α : Type u} {a : ℕ → α} {c : α},
   ...
   "body": [
      {
         ...
         "hypothesis": [
            ...
            {
               "type": "assume_statement",
               "assumption": "for all $n$, $a_{n+1} = a_n$"
            }
         ],
         "claim": "For all $n$, $a_n=c$.",
         "label": "thm:constant-sequence",
         "proof": {
            "type": "induction_proof",
            "on": "n",
            ...
            "base_case_proof": [
               {
                  "type": "assert_statement",
                  "claim": "$a_0 = c$",
                  "proof_method": "by the defining condition of the sequence",
                  "calculation": {
                     "type": "calculation",
                     "inline_calculation": "$a_0 = c$"
                  }
               },
               {
                  "type": "conclude_statement",
                  "claim": "The conclusion holds for $n=0$."
               }
            ],
            "induction_step_proof": [
               {
                  "type": "assume_statement",
                  "assumption": "for some arbitrary but fixed natural number $k$ we have $a_k = c$"
               },
               {
                  "type": "assert_statement",
                  "claim": "$a_{k+1} = a_k$",
                  "proof_method": "by the recursive definition of the sequence",
                  "calculation": {
                     "type": "calculation",
                     "inline_calculation": "$a_{k+1} = a_k$"
                  }
               },
               {
                  "type": "assert_statement",
               ...
               {
                  "type": "conclude_statement",
                  "claim": "The conclusion holds for $k+1$.",
                  "results_used": [
                     {
                        "statement": "$a_{k+1}=c$"
                     }
...
theorem sequence_const_of_zero_eq_c_and_succ_eq_self' :
      ∀ {α : Type u_1} (a : ℕ → α) (c : α),
        a (0 : ℕ) = c → (∀ (n : ℕ), a (n + (1 : ℕ)) = a n) → ∀ (n : ℕ), a n = c :=
    by
    intro α a c a_18193650970466190054 a_18068984259474765511 n
    induction n with
    | zero => grind only
    | succ k ih => grind only [#4b08]
    done
